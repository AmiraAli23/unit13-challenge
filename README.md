...
The objective of this assignment is to analyze the various cryptocurrencies available on the trading market.
...
Next, I removed any cryptocurrencies that had no coins mined. The values in the `TotalCoinsMined` column were either negative, 0, or positive. Since negative coins do not represent real value, I sorted the data to show all values > 0. 
...
Before standardizing the data, all "text" value columns, specifically, the `Algorithm` and `ProofType` columns, were converted into dummy variables.
